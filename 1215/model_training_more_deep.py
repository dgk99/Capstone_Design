...
class DrivingDataset(Dataset):
    def __init__(self, csv_file, transform=None):
        ...
        self.base_dir = "C:\\Users\\USER\\Desktop\\241215_resizing_curve3"  # 폴더 이름 수정
...
# 4. 데이터 로드 및 오버샘플링
csv_file = 'C:\\Users\\USER\\Desktop\\241215_resizing_curve3\\oversampled_data_12154.csv'  # CSV 파일 경로 수정
data = pd.read_csv(csv_file)

# 오버샘플링 수행
max_count = data['Steering Angle'].value_counts().max()
oversampled_data = []

for angle in data['Steering Angle'].unique():
    group = data[data['Steering Angle'] == angle]
...
data = pd.concat(oversampled_data).reset_index(drop=True)

# CSV로 저장 후 다시 로드
oversampled_csv_path = 'C:\\Users\\USER\\Desktop\\DGK\\mygithub\\Capstone_Design\\1215\\oversampled_final_12154.csv'
data.to_csv(oversampled_csv_path, index=False)

# 오버샘플링된 데이터로 데이터셋 생성
full_dataset = DrivingDataset(csv_file=oversampled_csv_path, transform=transform)
...
class ClassificationModel(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 32, kernel_size=5, stride=2, padding=2)  # 필터 수 증가
        self.conv2 = nn.Conv2d(32, 64, kernel_size=5, stride=2, padding=2)
        self.conv3 = nn.Conv2d(64, 128, kernel_size=5, stride=2, padding=2)  # 필터 및 레이어 추가
        self.conv4 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)
        self.conv5 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)
        
        self.fc1 = nn.Linear(128 * 8 * 8, 200)  # Fully Connected 레이어 증가
        self.fc2 = nn.Linear(200, 100)
        self.fc3 = nn.Linear(100, 5)  # 5개의 클래스 출력 (0~4)
...
# 최종 모델 저장
final_model_path = 'final_model_12155.pth'
