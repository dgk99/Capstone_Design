...
# CSV 파일 설정
csv_file = os.path.join(photo_dir, "steering_angles.csv")  # 사진과 같은 세션 디렉토리에 저장
photo_count = int(input("사진 번호를 몇 번부터 시작하시겠습니까? "))

# CSV 파일 초기화
with open(csv_file, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Filename", "Steering Angle"])  # CSV 헤더 작성
...
def capture_photos():
    ...
    while running:
        if is_recording.is_set():  # 주행 중일 때만 촬영
            ret, frame = cap.read()
            if ret:
                photo_filename = f"photo_{photo_count:05d}.jpg"  # 5자리 형식으로 번호 매기기
                photo_path = os.path.join(photo_dir, photo_filename)
                cv2.imwrite(photo_path, frame)
                
                # 조향 각도를 CSV에 기록
                with open(csv_file, mode='a', newline='') as file:  # 이어쓰기 모드
                    writer = csv.writer(file)
