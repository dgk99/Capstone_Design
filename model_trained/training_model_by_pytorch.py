...
class DrivingDataset(Dataset):
    def __init__(self, csv_file, transform=None):
        ...
        self.base_dir = "C:\\Users\\USER\\Desktop\\DGK\\mygithub\\Capstone_Design\\traning_data_set"  # 폴더 이름 수정
    ...
    def __getitem__(self, idx):
        ...
        # 이미지 경로가 상대 경로일 경우 절대 경로로 변환 (training_data_set 폴더 내에서 찾기)
...
dataset = DrivingDataset(csv_file='labeled_dataset.csv', transform=transform)
...
# 7. 모델 평가 (테스트/검증 데이터셋 사용)
model.eval()  # 평가 모드로 설정
with torch.no_grad():  # 기울기 계산 비활성화
    for images, steering_angles in dataloader:
        outputs = model(images)
        # 예측된 값과 실제 값을 비교하여 성능 평가 (예: MSE, R^2 등)
        # 여기서는 출력하지 않지만, 실제 프로젝트에서 필요한 평가를 추가할 수 있습니다.
